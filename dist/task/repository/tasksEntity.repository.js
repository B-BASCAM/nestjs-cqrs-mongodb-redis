"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TasksEntityRepository = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const task_entity_1 = require("../entities/task.entity");
const taskDetail_entity_1 = require("../entities/taskDetail.entity");
let TasksEntityRepository = class TasksEntityRepository {
    constructor(taskRepository, taskDetailRepository) {
        this.taskRepository = taskRepository;
        this.taskDetailRepository = taskDetailRepository;
    }
    async createTask(TaskEntity) {
        const result = await this.taskRepository.save(TaskEntity);
        return await this.taskRepository.findOneBy(result._id);
    }
    async findTaskById(id) {
        return await this.taskRepository.findOneBy(id);
    }
    async findTaskDetailByTaskId(options) {
        return await this.taskDetailRepository.find(options);
    }
    async updateOneTask(query, update) {
        return await this.taskRepository.updateOne(query, update);
    }
    async createManyTaskDetail(taskDetailEntityList) {
        return await this.taskDetailRepository.insert(taskDetailEntityList);
    }
};
TasksEntityRepository = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(task_entity_1.TaskEntity)),
    __param(1, (0, typeorm_1.InjectRepository)(taskDetail_entity_1.TaskDetailEntity)),
    __metadata("design:paramtypes", [typeorm_2.MongoRepository,
        typeorm_2.MongoRepository])
], TasksEntityRepository);
exports.TasksEntityRepository = TasksEntityRepository;
//# sourceMappingURL=tasksEntity.repository.js.map